...
export default function Top() {
  ...
  // データをタイプで分類
  const newsData = allData.filter((item) => item._sheetType === 'news').slice(0, 3)
  const liveData = allData.filter((item) => item._sheetType === 'live').slice(0, 3)
  const memberData = allData.filter((item) => item._sheetType === 'member')
  const musicData = allData.filter((item) => item._sheetType === 'music').slice(0, 3)
  ...
  useEffect(() => {
    if (contentRef.current && wrapperRef.current && allData.length > 0) {
      ...
      setTimeout(() => {
        ...
        const pixelsPerSecond = 150 // スクロール速度
        const duration = singleCycleHeight / pixelsPerSecond

        // 既存のスタイルを削除
        const existingStyle = document.getElementById('endroll-animation-style')
        if (existingStyle) {
  ...
  const EndrollContent = () => (
    <>
      ...
      <div className='endroll-section endroll-final'>
        ...
        <div className='endroll-copyright'>© 2025 Endroll. All rights reserved.</div>
        <div className='endroll-spacer'></div>
